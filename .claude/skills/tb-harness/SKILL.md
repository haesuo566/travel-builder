...
| 계획 커밋 뒤에 커밋이 있다 | "코드 변경 없음" / "취소됨" | 태스크가 진행됐다. **저널을 믿고 다시 구현하면 중복 작업이다** |
| `git status`가 더럽다 | 아무 말 없음 | 아래 분류를 거치기 전까지 재개하지 않는다 |

실제로 `findings-CD.md`의 Minor 3건이 저널의 "리뷰 미실행" 기록 때문에 **다음 세션까지 방치됐고, 사용자가 직접 짚어야 발견됐다.** 다른 실행에서는 저널이 "코드 변경 없음"이라 적었는데 Task 1이 이미 커밋돼 있었다.

**미커밋 소스 변경은 hunk 단위로 계획과 대조해 분류한다.** 두 종류가 겉모습이 같아서, 분류 없이 이어가면 어느 쪽이든 한 번에 잘못 확정된다.

| 근거 | 조치 |
|---|---|
| 계획의 어느 태스크 블록과 일치 | **진행분.** 검증(tsc·test·lint)을 처음부터 돌린 뒤 그 태스크로 커밋한다 — 중단된 세션이 검증까지 갔다는 증거는 없다 |
| 계획에도 HEAD에도 근거 없음 | **원복하고 저널에 남긴다.** 원인을 못 찾아도 "원인 불명이라 유지"는 선택지가 아니다 |

근거 없는 변경을 그대로 커밋했을 때의 실제 위험: `chat.service.ts` switch에서 두 의도의 핸들러 호출이 뒤바뀐 stray 변경이 있었다(reflog·stash에도 흔적 없음). 분기별 실제 응답이 아직 없어 **테스트는 전부 초록불이었다** — 검증으로는 잡히지 않는다.
