...
> **[실측 갱신 2 · 2026-07-28] 실행 완료.** 사용자가 `backend/.env`에 실제
> `GEMINI_API_KEY`를 넣은 뒤 `npm run start:dev`(포트 3001)로 전부 실행했다.
> **단, `GEMINI_MODEL=gemini-2.0-flash`로는 4건 전부 503(quota)이었다** — 이 키의
> 프로젝트에서 `gemini-2.0-flash`의 free tier 한도가 `limit: 0`이다(429
> RESOURCE_EXHAUSTED, 세 metric 전부 0). 키·모델 자체는 유효하다
> (`GET /v1beta/models`에 `gemini-2.0-flash` 존재). `GEMINI_MODEL=gemini-flash-latest`로
> 바꾸니 4건 전부 통과했다. **이 표의 아래 결과는 `gemini-flash-latest` 기준이다.**

- [x] **왕복이 성립한다** — "제주 2박3일 일정 짜줘" → **200**, `reply` 비어 있지 않음, 서버 로그에 Gemini 호출 오류 **0건**
- [x] **세 갈래가 실제로 갈린다** — ★**통과.** `reply` 3종이 서로 다르고, 각자 의도에 맞는 문구였다 — "일정을 새로 짜 드리는 기능은…"(`plan_itinerary`) / "여행지를 추천해 드리는 기능은…"(`recommend_places`) / "어디로 떠나고 싶으신가요?…"(`other`). 폴백이면 3건 모두 `other` 문구가 됐을 것이므로 **분류가 실제로 갈렸다는 증거다**
- [x] **폴백 로그 형식** — **미관측.** 4건 모두 정상 판정돼 `warn`이 한 줄도 남지 않았다. 억지로 만들지 않았다
- [x] **1001자 입력** → **400** (`message must be shorter than or equal to 1000 characters`). 경계값 1000자는 200으로 통과. Gemini 호출 없음의 근거는 **지연 0.004s** — 실제 왕복이 1.8~2.2s이므로 1/500이며 `ValidationPipe`가 컨트롤러 앞에서 끊은 것이다 (성공한 Gemini 호출은 로그를 남기지 않으므로 로그 부재만으로는 근거가 안 된다)
- [x] **응답 지연** — 기록만: 2.18s / 1.80s / 2.03s (20초 타임아웃 안)
