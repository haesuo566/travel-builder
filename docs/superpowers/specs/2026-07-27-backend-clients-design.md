...
#### 호출 인자로 받는 선택 값에도 같은 규칙을 적용한다

`opts.model` 같은 선택 파라미터가 빈 문자열이거나 **공백만** 있으면 기본값으로 폴백한다. `??`가 아니라 **`?.trim() ||`** 를 쓴다.

```ts
// 금지 — ''와 '  '를 유효한 값으로 받는다
opts.model ?? this.defaultModel

// 규약 — 부재·빈 문자열·공백을 모두 같게 다룬다
opts.model?.trim() || this.defaultModel
```

근거는 env와 같다. **"이름이 `''`인 리소스"는 존재하지 않으므로 `''`는 값이 아니라 부재의 표현이다.** 출처가 `.env`인지 호출 인자인지는 그 사실을 바꾸지 않는다. 대안은 빈 이름을 그대로 SDK에 실어 보내 502를 받는 것인데, 원인에서 먼 증상을 만들 뿐이다.

**`.trim()`이 필요한 이유는 실측으로 확인됐다.** `||`만으로는 공백이 통과한다 — 공백 문자열은 truthy다:

```
GEMINI_MODEL="  "  →  POST /v1beta/models/%20%20:generateContent   ★ 실제 요청이 나간다
```

**숫자·불리언 선택 값은 이 규칙에서 제외한다. `??`를 쓴다.** `0`과 `false`는 부재가 아니라 유효한 값이고, `||`는 그것들을 삼킨다. `temperature: 0`은 이 프로젝트가 실제로 쓰는 값이며(core의 구조화 호출이 `temperature: 0`이다), `||`로 다루면 조용히 기본값으로 바뀐다.

> **한 줄 요약:** 문자열 선택 값은 `?.trim() ||`, 숫자·불리언 선택 값은 `??`.

#### 이 규칙이 걸리는 곳

각 묶음이 같은 판단을 다시 하지 않도록 전부 열거한다.

| 값 | 종류 | 처리 | 묶음 |
|---|---|---|---|
| `GEMINI_MODEL` | env · 문자열 | `\|\|` → `gemini-2.0-flash` | B (완료) |
| `opts.model` | 호출 인자 · 문자열 | `?.trim() \|\|` → 기본 모델 | B (완료) |
| `opts.systemInstruction` | 호출 인자 · 문자열 | `?.trim() \|\| undefined` — 빈 지시문을 보내지 않는다 | B |
| `opts.temperature` | 호출 인자 · **숫자** | **`??`** — `0`이 유효하다 | B |
| `QDRANT_COLLECTION` | env · 문자열 | `\|\|` → `tour_contents` | **C** |
| `QDRANT_API_KEY` | env · 문자열 | `\|\|` → `undefined` (헤더 미전송) | **C** |
| `TourSearchFilter`의 각 필드 | 호출 인자 · 문자열 | `?.trim()`이 비면 **그 조건을 넣지 않는다.** `''`로 필터하면 아무것도 매치되지 않아 "결과 없음"이 된다 | **C** |
| `opts.limit` | 호출 인자 · **숫자** | **`??`** → 10 | **C** |
| TEI 임베딩 옵션 | — | **해당 없음.** 옵션을 노출하지 않기로 했다 | D |
| `embedQuery(text)` | **필수** 인자 · 문자열 | 폴백할 기본값이 없다 → 빈/공백은 `invalid-request`로 거부 | D |

마지막 두 행이 규칙의 경계를 보여준다. 같은 "빈 문자열은 부재다"라는 판단에서 출발하지만, **부재를 메울 기본값이 있으면 폴백하고 없으면 거부한다.** `embedQuery`의 빈 질의를 조용히 무언가로 대체하면 사용자가 요청하지 않은 검색을 하게 된다.
...
| 상태 | kind |
|---|---|
| **404** | **`not-found`** (엔드포인트 경로 오설정 — 우리 `.env`의 문제다. 아래 절 참조) |
...
#### 404를 `not-found`(500)로 끊는다 — 초판의 "`not-found`는 TEI에 없다"를 정정한다

초판은 "`auth`·`quota`·`not-found`는 TEI에 없다"고 한 줄로 선언했다. **셋 중 근거가 있는 것은 앞의 둘뿐이었다.**

- **`auth`가 없다 — 근거 있음.** 자체 호스팅 TEI는 인증 없이 동작하고(`:256`, `2026-07-23-core-tei-embedding-client-design.md:16`), 우리 env에는 TEI 자격증명이 애초에 없다. 프록시가 앞에 붙어 401/403을 내는 배포라면 그건 `:256`의 전제 자체가 깨진 상황이고, 그때 필요한 것은 분류기 한 줄이 아니라 TEI 인증을 설정에 추가하는 spec 변경이다. 그래서 401/403은 지금 "그 외 비-2xx"로 흘려보낸다.
- **`quota`가 없다 — 근거 있음.** TEI에는 호출자별 예산이라는 개념이 없다. TEI의 429는 모델 과부하이며 쿼터 소진이 아니라 용량 부족이므로 `upstream`이 맞다. Gemini 429와 달리 기다림의 단위가 초이고, `Retry-After`를 우리가 지어낼 근거도 없다.
- **`not-found`가 없다 — 근거가 없었다.** `:256`은 **인증의 부재만** 설명한다. 에러 표에도 TEI 404 행 자체가 없었다. 결정이 아니라 누락이다.

`:628`이 이미 이 경우를 금지한다 — "모델명 오타와 컬렉션 이름 오타는 같은 종류의 오설정이므로 같은 kind여야 한다." **엔드포인트 경로 오설정도 같은 종류다.** `TEI_BASE_URL=http://tei.internal:8080/v1`처럼 경로를 한 칸 더 적으면 `POST .../v1/embed`가 404를 받고, 초판 규칙에서는 `upstream`(502) "외부 서비스에서 오류가 발생했습니다."가 된다. **TEI는 멀쩡한데 그 응답과 로그를 받은 사람은 TEI 장애를 조사한다** — Gemini 404에 대해 `:626`이 거부한 것과 글자 하나 다르지 않은 상황이다.

**판정의 실질적 근거는 404가 요청 내용과 무관하게 불변이라는 것이다.** 경로 `{TEI_BASE_URL}/embed`는 코드가 고정하고, 바디는 `{ inputs: [text], normalize: true, truncate: true }`뿐이다(`:657`). 어떤 질의도, 어떤 부하도 404를 만들 수 없다 — 입력 쪽 실패는 400/413/422로, 모델·서버 쪽 실패는 5xx로 온다. 즉 404는 **모든 요청에 똑같이 재현되는 배선 불일치**이고, 불변인 실패의 책임은 워크로드가 아니라 설정에 있다. 재시도가 무의미하다는 사실도 502/503이 아니라 500이어야 하는 이유다 — 503은 "나중에 다시"라는 거짓말이 된다(`:972`와 같은 논리).

**반론: TEI 배포 쪽이 라우트를 옮겼다면 틀린 것은 우리 `.env`가 아니다.** 맞다. 그러나 어느 쪽이 원인이든 증상은 "도달할 라우트가 없다"는 배선 불일치이고, 우리가 고칠 수 있는 쪽은 `TEI_BASE_URL` 한 곳뿐이다. 500은 온콜을 그 한 곳으로 보낸다. 502는 고칠 수 없는 쪽(TEI 헬스)을 먼저 보게 만든다. 이 반론은 kind를 뒤집지 못하고, 로그 문구가 "엔드포인트를 찾을 수 없음"임을 분명히 해야 한다는 요구로만 남는다.

**이 정정은 Task 10의 구조 검증 기준을 깨지 않는다.** `not-found`는 이미 `ExternalFailureKind`에 있고, `MESSAGE_BY_KIND['not-found']`("외부 서비스에서 대상을 찾을 수 없습니다.")는 엔드포인트 부재에도 맞는 문구다. **공통 파일은 여전히 유니온 한 줄 외에 바뀌지 않는다.**

따라서 확정 문장은 이렇게 바뀐다: **`auth`·`quota`는 TEI에 없고, `not-found`는 있다.**
...
| **TEI 404 / `TEI_BASE_URL` 경로 오설정** | **우리 설정** | `not-found` | **500** | 없음 | error (요청 URL 기록) |
...
**`test-asymmetry.md`** — 위 에러 표는 25행이다. 테스트 목록도 행마다 짝을 맞춘다. 특히 겉모습이 같아서 한쪽만 테스트하기 쉬운 쌍을 명시적으로 짝짓는다: "hit 0건은 실패가 아니다" ↔ "hit은 있는데 전 건 파싱 실패는 실패다", "TEI 빈 응답은 실패다" ↔ "정상 벡터는 그대로 통과한다", **"TEI 404는 우리 설정(`not-found`/500)이다" ↔ "그 밖의 분류되지 않은 비-2xx는 외부(`upstream`/502)다"** — 이 쌍은 "분류되지 않은 비-2xx는 upstream"이라는 케이스 하나가 404까지 함께 못 박아버리기 쉬우므로 반드시 두 케이스로 분리한다.
