...
export function createEnricher(
   ...
   * 이름과 달리 "연속"을 항목 단위로 엄밀히 추적하지 않는다 — 성공한 Gemini 호출
   * 뒤에서만 리셋되므로(:204 부근), 중간의 비-429 오류나 structuredText 재사용으로
   * Gemini를 아예 안 부른 항목은 리셋도 증가도 하지 않는다. 즉 429, 429, 500(구조화
   * 실패), 429도 이 카운터엔 "연속 3회"로 보인다. 쿼터 단축회로를 더 보수적으로(일찍)
   * 트립시키는 방향이라 무해하지만, 문턱값을 튜닝할 때는 이 점을 감안해야 한다.
   */
